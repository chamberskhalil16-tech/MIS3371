...
function validateLastName() 
{
    ...
    //Ensures proper characters are being used
        ...
        {document.getElementById("Last Name-error").innerHTML = "Only Letters, dashes, and apostrophes are allowed";
        return false;}
    //If all other statements are passed, will clear error messages and allow a pass through
    else 
...
//Sets timelimit for how long cookie ie stored
function setCookie(name, lastvalue, duration) {
    var day = new Date();
    day.setTime(day.getTime() + (duration * 24 * 60 * 60 * 1000));
    var expires = "expires=" + day.toUTCString();
    document.cookie = name + "=" + encodeURIComponent(lastvalue) + ";" + expires + ";path=/";
}

//Gets cookie when user returns to page
...
[
    ...
    {id:"FirstN", cookieName: "firstName"},
    {id:"MiddleI", cookieName: "Middle Initial"},
    {id:"LastN", cookieName: "Last Name"},
    ...
    {id:"UserID", cookieName: "User Name"},
...
if (firstName !== "") {
    document.getElementById("Return1").innerHTML = "Welcome back, " + firstName + "!<br>";
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const rememberMeElement = document.getElementById("Save-Info");
...
// Function to fetch and populate states via fetchAPIs
async function loadStates() {
    const stateSelect = document.getElementById("state")
    
    try {
        // will Retrieve the states list from the local file
        const response = await fetch('statelist.json')
        const states = await response.json();
        
        stateSelect.innerHTML = '<option selected disabled>Please Select One</option>'
